...
def attribute_names():
    return properties + \
           ["entrypoint%d" % i for i in range(64)] + \
           ["byte(%02x)" % b for b in range(256)] + \
...
def calculate_byte_histogram(file_path):
    try:
        ...
        print(f"Error calculating byte histogram for {file_path}: {e}")
        return np.zeros(256)
...
def encode_pe_file(file_path):
    ...
    if hasattr(pe, 'imports'):
        ...
        features.extend([0.0] * len(libraries))

    # Encode sections information
    try:
        sections = pe.sections
        if sections:
            features.append(len(sections))  # Number of sections
            features.append(sum(s.entropy for s in sections) / len(sections))  # Average entropy
            features.append(sum(s.size for s in sections) / len(sections))  # Average size
            features.append(sum(s.virtual_size for s in sections) / len(sections))  # Average virtual size
        else:
            features.extend([0, 0, 0, 0])  # Default values
    except Exception as e:
        print(f"Error processing sections for {file_path}: {e}")
    ...
    try:
        ...
        print(f"Error calculating virtual size ratio for {file_path}: {e}")
        features.append(0.0)  # Default value for virtual size ratio

    return np.array(features)

# Define your directories
malware_dir = 'E:/combined_dataset/validation/malware'
benign_dir = 'E:/combined_dataset/validation/benign'

# Function to process a directory of PE files and write the features to a CSV
...
# Open a CSV file to write the features
with open('../data/validation_extracted_features.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    # Write a header row
    writer.writerow(attribute_names() + ['label'])
    
    # Process malware files
    process_directory(malware_dir, 'malware', writer)
